...
protected:
  TestLRF(double angle_min_ = -M_PI, double angle_max_ = M_PI,
          double range_min_ = 0.2, double range_max_ = 60., uint n_beams_ = 9)
      : angle_min(angle_min_), angle_max(angle_max_), range_min(range_min_),
        range_max(range_max_), n_beams(n_beams_) {}
...
TEST_F(TestLRF3, renderLineWeird) {
  double a1 = angle_max -
              0.1; // Point in view of the robot -> angle_min < a1 < angle_max
  double a2 = angle_min - 1.0; // Point in the blind spot of the robot but with
                               // a positive angle -> a2 > 0

  // The line connecting these two points should pass behind the robot -> a1 -
  // a2 > M_PI However the difference between the a1 and the lower angle limit
  // is less than half a circle -> a1 + a_max < M_PI
  ...
  // The rendered line passes behind the robot. But not through the blindspot.
  // Therefore the first index should remain untouched
  for (uint i = 0; i < upper_index_a1 - 1; ++i) {
    ASSERT_EQ(ranges[i], range_max)
        << "Range at index [" << i
        << "] should not be rendered. Instead is rendered to " << ranges[i];
  }
  // At a1 we should see the switch between rendered and not rendered. indices
  // above a1 should be rendered. Because points lie on a unit circle they
  // should be <= 1
  for (uint i = upper_index_a1; i < n_beams; ++i) {
    ASSERT_LE(ranges[i], 1.)
        << "Range at index [" << i
...
TEST_F(TestLRF3, renderLineRight2) {
  ...
  for (uint i = 0; i < upper_index_p2 - 1; ++i) {
    ASSERT_LE(ranges[i], sqrt(2))
        << "Range at index [" << i << "] should be rendered to <=" << sqrt(2)
...
TEST_F(TestLRF2, getAngleUpperIndexAngle) {
  ...
  ASSERT_EQ(lrf.getAngleUpperIndex(0.5 * angle_min + 0.5 * angle_max),
...
TEST_F(TestLRF2, getAngleUpperIndexXY) {
  ASSERT_EQ(lrf.getAngleUpperIndex(cos(1.5 * angle_min), sin(1.5 * angle_min)),
            0u);
  ASSERT_EQ(lrf.getAngleUpperIndex(cos(angle_min), sin(angle_min)), 1u);
  ASSERT_EQ(lrf.getAngleUpperIndex(cos(0.5 * angle_min + 0.5 * angle_max),
                                   sin(0.5 * angle_min + 0.5 * angle_max)),
            std::floor(0.5 * n_beams - 0.5) + 1);
  ASSERT_EQ(lrf.getAngleUpperIndex(cos(angle_max), sin(angle_max)), n_beams);
  ASSERT_EQ(lrf.getAngleUpperIndex(cos(1.5 * angle_max), sin(1.5 * angle_max)),
            n_beams);
}
