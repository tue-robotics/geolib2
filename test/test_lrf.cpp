...
{
protected:
    TestLRF(double angle_min_ = -M_PI,
...
TEST_F(TestLRF3, renderLineWeird)
{
    ...
    {
        ASSERT_EQ(ranges[i], range_max) << "Range at index [" << i
    ...
    {
        ASSERT_LE(ranges[i], 1.) << "Range at index [" << i << "] should be rendered to <=1. Instead its value is "
                                 << ranges[i];
    }
}
...
TEST_F(TestLRF3, renderLineRight2)
{
    ...
    {
        ASSERT_LE(ranges[i], sqrt(2)) << "Range at index [" << i << "] should be rendered to <=" << sqrt(2)
...
TEST_F(TestLRF2, getAngleUpperIndexXY)
{
    ...
    ASSERT_EQ(lrf.getAngleUpperIndex(cos(0.5 * angle_min + 0.5 * angle_max), sin(0.5 * angle_min + 0.5 * angle_max)),
