...
{
    geo::DepthCamera cam(CANVAS_WIDTH,
                         CANVAS_HEIGHT,
...
int main(int argc, char** argv)
{
    ...
    for (double v = 0; v < 1; v += res2)
    {
        axis.add(geo::Vector3(v, 0, 0));
    }
    for (double v = 0; v < 1; v += res2 * 2)
    {
        axis.add(geo::Vector3(0, v, 0));
    }
    for (double v = 0; v < 1; v += res2 * 4)
    {
        axis.add(geo::Vector3(0, 0, v));
    }
    ...
    //    std::cout << "DepthCamera::rasterize(abstract_shape):\t" << renderDepthCamera(image, tree, true, false) << "
    //    ms" << std::endl;

    if (mesh)
    {
        std::cout << "DepthCamera::rasterize(input_mesh):\t" << renderDepthCamera(image, *mesh, false) << " ms"
                  << std::endl;
    }
    ...
    //    std::cout << "DepthCamera::rasterize(abstract_shape):\t" << renderLRF(image, tree, true, false) << " ms" <<
    //    std::endl;

    if (mesh)
    {
        std::cout << "LaserRangeFinder::rasterize(input_mesh):\t" << renderLRF(image, *mesh, false) << " ms"
                  << std::endl;
    }
    ...
    while (true)
    {
        ...
        lrf.render(geo::Box(geo::Vector3(-0.5, -3.5, -0.5), geo::Vector3(0.5, 3.5, 0.5)),
                   geo::Pose3D(0, 0, 0),
                   geo::Pose3D(-2, 0, 0),
                   ranges);
        ...
        cv::Mat destinationROI2 =
            display_image(cv::Rect(cv::Point(CANVAS_WIDTH, 0), cv::Size(CANVAS_WIDTH, CANVAS_HEIGHT)));
